...
class ThreatClassifier:
    ...
    def extract_features_for_trained_model(self, log_entry):
        ...
        features = {
            ...
            'has_network_activity': 1 if log_entry.get('network_connections') or log_entry.get('connections') else 0,
            'connection_count': int(log_entry.get('network_connections') or log_entry.get('connections') or 0),
            'is_system_user': 1 if username in ['system', 'root', 'nt authority\\system'] else 0,
            'has_encoded_cmd': 1 if any(p in cmdline for p in encoded_patterns) else 0,
            'has_download_cmd': 1 if any(p in cmdline for p in download_patterns) else 0,
            'has_connect_cmd': 1 if any(p in cmdline for p in connect_patterns) else 0,
            'has_encrypt_cmd': 1 if any(p in cmdline for p in encrypt_patterns) else 0,
            'has_cred_dump': 1 if any(p in cmdline or p in process_name for p in cred_dump_patterns) else 0,
            ...
            'has_lateral_movement': 1 if any(p in cmdline for p in lateral_patterns) else 0,
            'is_procdump': 1 if 'procdump' in process_name or 'procdump' in cmdline else 0,
        }
